...
CONTENTS=$(INPUTDIR)/$(FILENAME).mkd
...
html5:
	...
	sed -if '/<\/header>/s/<\/header>/<\/header><input type="checkbox" id="nav-toggle" class="nav-toggle">/g' $(FILENAME).html
	sed -if '/<\/nav>/s/<\/nav>/<\/nav><label for="nav-toggle" class="nav-toggle-label"><span id="square"><\/span><\/label>/g' $(FILENAME).html
	rm $(FILENAME).htmlf
